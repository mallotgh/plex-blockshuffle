 ...
 *  - Blöcke gehören einem Nutzer; fremde Blöcke sind unsichtbar (404).
...
function playlistTrackIds(db: DB, userId: string, playlistId: string): Set<string> {
  const rows = db
    .prepare('SELECT DISTINCT track_id FROM playlist_items WHERE user_id = ? AND playlist_id = ?')
    .all(userId, playlistId) as { track_id: string }[];
  return new Set(rows.map((r) => r.track_id));
}

function playlistPosition(db: DB, userId: string, playlistId: string): Map<string, number> {
  const rows = db
    .prepare(
      'SELECT track_id, MIN(position) AS pos FROM playlist_items WHERE user_id = ? AND playlist_id = ? GROUP BY track_id',
    )
    .all(userId, playlistId) as { track_id: string; pos: number }[];
  return new Map(rows.map((r) => [r.track_id, r.pos]));
}

export function getBlocks(db: DB, userId: string, playlistId: string): BlockWithItems[] {
  const inPlaylist = playlistTrackIds(db, userId, playlistId);
  const blocks = db
    .prepare('SELECT * FROM blocks WHERE user_id = ? AND playlist_id = ? ORDER BY created_at, id')
    .all(userId, playlistId) as BlockRow[];
  const itemRows = db
    .prepare(
      `SELECT bi.block_id, bi.track_id, bi.position FROM block_items bi
       JOIN blocks b ON b.id = bi.block_id
       WHERE b.user_id = ? AND b.playlist_id = ? ORDER BY bi.block_id, bi.position`,
    )
    .all(userId, playlistId) as { block_id: string; track_id: string; position: number }[];
...
export function getBlock(db: DB, userId: string, blockId: string): BlockRow {
  const block = db
    .prepare('SELECT * FROM blocks WHERE id = ? AND user_id = ?')
    .get(blockId, userId) as BlockRow | undefined;
  if (!block) throw new ApiError(404, 'block_not_found', 'Block nicht gefunden.');
  return block;
}

export function getBlockWithItems(db: DB, userId: string, blockId: string): BlockWithItems {
  const block = getBlock(db, userId, blockId);
  return getBlocks(db, userId, block.playlist_id).find((b) => b.id === blockId)!;
}

function nextColor(db: DB, userId: string, playlistId: string): string {
  const used = (
    db.prepare('SELECT color FROM blocks WHERE user_id = ? AND playlist_id = ?').all(userId, playlistId) as {
      color: string;
    }[]
  ).map((r) => r.color);
...
export function createBlock(
  ...
  return db.transaction(() => {
    ...
    const pos = playlistPosition(db, userId, playlistId);
...
export function updateBlock(
  ...
  return getBlock(db, userId, blockId);
}

/** Setzt die komplette (Reihenfolge der) Trackliste eines Blocks. */
export function setBlockItems(db: DB, userId: string, blockId: string, trackIds: string[]): BlockWithItems {
  const block = getBlock(db, userId, blockId);
  const unique = [...new Set(trackIds)];
  if (unique.length < 2) {
    throw new ApiError(400, 'block_too_small', 'Ein Block braucht mindestens 2 Tracks.');
  ...
  const inPlaylist = playlistTrackIds(db, userId, block.playlist_id);
  ...
  return db.transaction(() => {
    ...
    return getBlockWithItems(db, userId, blockId);
  })();
}
...
export function removeTrackFromBlock(db: DB, userId: string, blockId: string, trackId: string): RemoveResult {
  getBlock(db, userId, blockId);
  return db.transaction(() => {
...
export function addTrackToBlock(db: DB, userId: string, blockId: string, trackId: string): BlockWithItems {
  const block = getBlock(db, userId, blockId);
  const inPlaylist = playlistTrackIds(db, userId, block.playlist_id);
  ...
  return db.transaction(() => {
    ...
    db.prepare('INSERT OR IGNORE INTO block_items (block_id, track_id, position) VALUES (?, ?, ?)').run(
      blockId,
      trackId,
      max + 1,
    );
...
export function deleteBlock(db: DB, userId: string, blockId: string): void {
  getBlock(db, userId, blockId);
  db.prepare('DELETE FROM blocks WHERE id = ?').run(blockId);
}
