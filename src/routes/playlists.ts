...
export function requirePlaylist(db: DB, userId: string, id: string): PlaylistRow {
  const row = db
    .prepare('SELECT * FROM playlists WHERE user_id = ? AND id = ?')
    .get(userId, id) as PlaylistRow | undefined;
  if (!row) throw new ApiError(404, 'playlist_not_found', 'Playlist nicht gefunden.');
  return row;
}

export function playlistDetail(db: DB, userId: string, playlistId: string) {
  const playlist = requirePlaylist(db, userId, playlistId);
  const items = db
    .prepare(
      `SELECT pi.position, t.* FROM playlist_items pi
       JOIN tracks t ON t.id = pi.track_id
       WHERE pi.user_id = ? AND pi.playlist_id = ? ORDER BY pi.position`,
    )
    .all(userId, playlistId) as ({ position: number } & TrackRow)[];
  const blocks = getBlocks(db, userId, playlistId);
...
export function registerPlaylistRoutes(app: FastifyInstance, ctx: AppContext): void {
  app.get('/api/playlists', async (req) => {
    const { userId, plex } = requireUser(req, ctx);
    const query = refreshQuerySchema.parse(req.query);
    const count = (
      ctx.db.prepare('SELECT COUNT(*) AS n FROM playlists WHERE user_id = ? AND stale = 0').get(userId) as {
        n: number;
      }
    ).n;
    if (query.refresh || count === 0) {
      await syncPlaylists(ctx.db, plex);
    }
    const rows = ctx.db
      .prepare(
        `SELECT p.*, (SELECT COUNT(*) FROM blocks b WHERE b.user_id = p.user_id AND b.playlist_id = p.id) AS block_count
         FROM playlists p
         WHERE p.user_id = ? AND p.stale = 0
           AND p.id NOT IN (SELECT shadow_playlist_id FROM shadow_playlists WHERE user_id = ?)
         ORDER BY (block_count > 0) DESC, p.name COLLATE NOCASE`,
      )
      .all(userId, userId) as (PlaylistRow & { block_count: number })[];
    return {
  ...
  app.get('/api/playlists/:id', async (req) => {
    ...
    const query = refreshQuerySchema.parse(req.query);
    await syncPlaylistItems(ctx.db, plex, id, { force: query.refresh });
    return playlistDetail(ctx.db, userId, id);
  });
}
