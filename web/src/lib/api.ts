import type { AuthStatus, PlaylistDetail, PlaylistSummary, ShuffleRun } from '../types';
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    ...
    // Session serverseitig weg -> zurück zum Login
...
export const api = {
  ...
  importBlocks: (data: unknown) =>
    request<{ imported: number; skipped: { playlist: string; block: string; reason: string }[] }>(
      '/api/import/blocks',
